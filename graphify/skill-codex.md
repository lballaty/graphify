...
This writes a `## graphify` section to the local `AGENTS.md` that instructs Codex to check `graphify-out/index.json` first, then use the graph outputs before answering codebase questions. No manual `$graphify` needed in future sessions for routine graph-aware navigation.
